...
let phraseChoosed = Math.round(Math.random() * phrases.length)
const cookie = document.getElementById("cookieClick")
const box1 = document.querySelector(".box1")
const box2 = document.querySelector(".box2")
const btnAnotherCookie = document.getElementById("btnAnotherCookie")

function handleClickCookie() {
  toggleBox()
  box2.querySelector("p").innerText = phrases[phraseChoosed]
}

function handleResetCookie() {
  phraseChoosed = Math.round(Math.random() * phrases.length)
  toggleBox()
}

function enterPressed(e) {
  if (e.key == "Enter" && box2.classList.contains("hide")) {
    handleClickCookie()
  } else if (e.key == "Enter" && box1.classList.contains("hide")) {
    handleResetCookie()
  }
}
...
btnAnotherCookie.addEventListener("click", handleResetCookie)
